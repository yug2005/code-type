...
export const Slider = (props:SliderInterface) => {
    const updateSlider = () => {
        ...
        if (slider_value && width) {
            slider_value.style.left = `${40 + (width - 15) * (props.range - props.min) / (props.max - props.min)}px`
        }
    }
    ...
    return (
        <div className='slider'>
            <div className="slider-field">
                ...
                <input type="range" className="input-slider" id={`${props.id}-slider`} 
                    min={props.min} max={props.max} value={props.range} step={props.step}
                    onChange={(e) => props.setRange(parseInt(e.target.value))}
                />
                <div className="slider-right-value">{props.max}</div>
            </div>
            <div id={`${props.id}-slider-value`} className="slider-value">{props.range}</div>
        </div>
    )
}